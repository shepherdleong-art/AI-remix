...
const VideoPreviewDialog: React.FC<VideoPreviewDialogProps> = ({
  ...
  // In browser dev mode, backendUrl is '' (relative URL, Vite proxy handles routing)
  // In Electron mode, backendUrl is 'http://127.0.0.1:${port}'
  const videoUrl: string = job.outputPath
    ? `${backendUrl}/api/render/output/${job.id}`
    : '';
